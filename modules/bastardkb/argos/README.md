...
Argos is a QMK community module that provides dynamic configuration of advanced QMK features via USB HID protocol. It's designed to work with the Vial GUI (or compatible tools) to allow users to configure tap dance, combos, key overrides, and QMK settings without recompiling firmware.
...
Argos will automatically use `VIAL_KEYBOARD_UID` if defined. You can also define `ARGOS_KEYBOARD_UID` directly:

```c
#define ARGOS_KEYBOARD_UID {0x1B, 0x18, 0x7D, 0xF2, 0x21, 0xF6, 0x29, 0x48}
```
...
#define ARGOS_TAP_DANCE_ENTRIES 50

// Combo entries (default: 16, each uses 12 bytes)
#define ARGOS_COMBO_ENTRIES 50

// Key override entries (default: 16, each uses 12 bytes)
#define ARGOS_KEY_OVERRIDE_ENTRIES 30
...
Argos stores data in the keyboard's EEPROM datablock. The layout is:
...
Argos uses the build timestamp as a version marker. When firmware is flashed, if the timestamp differs from what's stored in EEPROM, all Argos settings are reset to defaults. This ensures a clean state when firmware changes might affect data layout.

## Protocol

Argos uses the `0xDF` prefix for its USB HID protocol, separate from VIA's `0xFE` prefix. This allows it to coexist with VIA while providing additional functionality.

## GUI Support

Argos is designed to work with the Vial GUI. The GUI auto-detects Argos support via the `argos:` prefix in the USB serial string.
