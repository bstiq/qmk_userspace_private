...
static argos_alt_repeat_key_entry_t argos_alt_repeat_entries[ARGOS_ALT_REPEAT_KEY_ENTRIES];

// Track which entries are enabled
static bool argos_ark_entry_enabled[ARGOS_ALT_REPEAT_KEY_ENTRIES];

void argos_reload_alt_repeat_key(void) {
    memset(argos_alt_repeat_entries, 0, sizeof(argos_alt_repeat_entries));
    memset(argos_ark_entry_enabled, 0, sizeof(argos_ark_entry_enabled));

    // Load from EEPROM
    for (size_t i = 0; i < ARGOS_ALT_REPEAT_KEY_ENTRIES; ++i) {
        argos_alt_repeat_key_entry_t entry;
        if (argos_get_alt_repeat_key(i, &entry) == 0) {
            memcpy(&argos_alt_repeat_entries[i], &entry, sizeof(entry));
            argos_ark_entry_enabled[i] = (entry.options & argos_ark_enabled) != 0;
        }
    }
}
...
uint16_t argos_get_alt_repeat_keycode(uint16_t keycode, uint8_t mods) {
    for (size_t i = 0; i < ARGOS_ALT_REPEAT_KEY_ENTRIES; ++i) {
        if (!argos_ark_entry_enabled[i]) continue;

        argos_alt_repeat_key_entry_t *entry = &argos_alt_repeat_entries[i];
        ...
        if (entry->options & argos_ark_option_ignore_mod_handedness) {
        ...
        // Found a match
        bool default_to_alt = (entry->options & argos_ark_option_default_to_alt) != 0;
...
uint16_t argos_get_reverse_alt_repeat_keycode(uint16_t keycode, uint8_t mods) {
    for (size_t i = 0; i < ARGOS_ALT_REPEAT_KEY_ENTRIES; ++i) {
        if (!argos_ark_entry_enabled[i]) continue;
        ...
        if (!(entry->options & argos_ark_option_bidirectional)) continue;
