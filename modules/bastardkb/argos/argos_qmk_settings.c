...
} argos_qmk_settings_t;
_Static_assert(sizeof(argos_qmk_settings_t) == 40, "argos_qmk_settings_t must be 40 bytes");

// RAM copy of settings
static argos_qmk_settings_t settings;

// Forward declarations
static void argos_qmk_settings_load(void);
static void argos_qmk_settings_save(void);
static void argos_qmk_settings_apply(void);
...
} argos_setting_desc_t;

// Setting descriptors - defines all supported QSIDs
static const argos_setting_desc_t setting_descs[] = {
    { QSID_GRAVE_ESC_OVERRIDE, 1, offsetof(argos_qmk_settings_t, grave_esc_override), 0 },
    { QSID_COMBO_TERM, 2, offsetof(argos_qmk_settings_t, combo_term), 0 },
    { QSID_AUTO_SHIFT, 1, offsetof(argos_qmk_settings_t, auto_shift), 0 },
    { QSID_AUTO_SHIFT_TIMEOUT, 2, offsetof(argos_qmk_settings_t, auto_shift_timeout), 0 },
    { QSID_OSK_TAP_TOGGLE, 1, offsetof(argos_qmk_settings_t, osk_tap_toggle), 0 },
    { QSID_OSK_TIMEOUT, 2, offsetof(argos_qmk_settings_t, osk_timeout), 0 },
    { QSID_TAPPING_TERM, 2, offsetof(argos_qmk_settings_t, tapping_term), 0 },
#ifdef MOUSEKEY_ENABLE
    { QSID_MOUSEKEY_DELAY, 2, offsetof(argos_qmk_settings_t, mousekey_delay), 0 },
    { QSID_MOUSEKEY_INTERVAL, 2, offsetof(argos_qmk_settings_t, mousekey_interval), 0 },
    { QSID_MOUSEKEY_MOVE_DELTA, 2, offsetof(argos_qmk_settings_t, mousekey_move_delta), 0 },
    { QSID_MOUSEKEY_MAX_SPEED, 2, offsetof(argos_qmk_settings_t, mousekey_max_speed), 0 },
    { QSID_MOUSEKEY_TIME_TO_MAX, 2, offsetof(argos_qmk_settings_t, mousekey_time_to_max), 0 },
    { QSID_MOUSEKEY_WHEEL_DELAY, 2, offsetof(argos_qmk_settings_t, mousekey_wheel_delay), 0 },
    { QSID_MOUSEKEY_WHEEL_INTERVAL, 2, offsetof(argos_qmk_settings_t, mousekey_wheel_interval), 0 },
    { QSID_MOUSEKEY_WHEEL_MAX_SPEED, 2, offsetof(argos_qmk_settings_t, mousekey_wheel_max_speed), 0 },
    { QSID_MOUSEKEY_WHEEL_TIME_TO_MAX, 2, offsetof(argos_qmk_settings_t, mousekey_wheel_time_to_max), 0 },
#endif
    { QSID_TAP_CODE_DELAY, 2, offsetof(argos_qmk_settings_t, tap_code_delay), 0 },
    { QSID_TAP_HOLD_CAPS_DELAY, 2, offsetof(argos_qmk_settings_t, tap_hold_caps_delay), 0 },
    { QSID_TAPPING_TOGGLE, 1, offsetof(argos_qmk_settings_t, tapping_toggle), 0 },
    { QSID_MAGIC, 0, 0, 0 },  // Special handler
    // Individual bit settings from tapping_v2
    { QSID_PERMISSIVE_HOLD, 1, offsetof(argos_qmk_settings_t, tapping_v2), TAPPING_PERMISSIVE_HOLD_BIT },
    { QSID_HOLD_ON_OTHER_KEY, 1, offsetof(argos_qmk_settings_t, tapping_v2), TAPPING_HOLD_ON_OTHER_KEY_BIT },
    { QSID_RETRO_TAPPING, 1, offsetof(argos_qmk_settings_t, tapping_v2), TAPPING_RETRO_TAPPING_BIT },
    { QSID_QUICK_TAP_TERM, 2, offsetof(argos_qmk_settings_t, quick_tap_term), 0 },
    { QSID_CHORDAL_HOLD, 1, offsetof(argos_qmk_settings_t, tapping_v2), TAPPING_CHORDAL_HOLD_BIT },
    { QSID_FLOW_TAP_TERM, 2, offsetof(argos_qmk_settings_t, flow_tap_term), 0 },
};

#define NUM_SETTINGS (sizeof(setting_descs) / sizeof(setting_descs[0]))

// Find setting descriptor by QSID
static const argos_setting_desc_t* find_setting(uint16_t qsid) {
...
void argos_qmk_settings_init(void) {
    argos_qmk_settings_load();
    argos_qmk_settings_apply();
}

// Load settings from EEPROM
static void argos_qmk_settings_load(void) {
    argos_read_eeprom(ARGOS_QMK_SETTINGS_OFFSET, &settings, sizeof(settings));
}

// Save settings to EEPROM
static void argos_qmk_settings_save(void) {
    argos_write_eeprom(ARGOS_QMK_SETTINGS_OFFSET, &settings, sizeof(settings));
}
...
int argos_qmk_settings_get(uint16_t qsid, uint8_t *buffer, uint8_t length) {
    ...
    const argos_setting_desc_t *desc = find_setting(qsid);
    if (!desc) return -1;
...
#if defined(ARGOS_DEFAULT_PERMISSIVE_HOLD) && ARGOS_DEFAULT_PERMISSIVE_HOLD
    settings.tapping_v2 |= (1 << TAPPING_PERMISSIVE_HOLD_BIT);
#endif
#if defined(ARGOS_DEFAULT_CHORDAL_HOLD) && ARGOS_DEFAULT_CHORDAL_HOLD
    settings.tapping_v2 |= (1 << TAPPING_CHORDAL_HOLD_BIT);
#endif
#if defined(ARGOS_DEFAULT_HOLD_ON_OTHER_KEY) && ARGOS_DEFAULT_HOLD_ON_OTHER_KEY
    settings.tapping_v2 |= (1 << TAPPING_HOLD_ON_OTHER_KEY_BIT);
#endif
#if defined(ARGOS_DEFAULT_RETRO_TAPPING) && ARGOS_DEFAULT_RETRO_TAPPING
...
#ifdef NKRO_ENABLE
#   if defined(ARGOS_DEFAULT_NKRO)
    keymap_config.nkro = ARGOS_DEFAULT_NKRO;
#   else
...
// User hooks (*_argos suffix): override these for custom per-key logic
// For booleans: return -1 to use Argos's setting, 0 for false, 1 for true
// For uint16_t: return 0 to use Argos's setting, or a positive value to override
// User hooks are checked FIRST - they always beat Argos's settings
// Note: get_tapping_term_argos is in argos_tap_dance.c
// Note: get_combo_term_argos is in argos_combo.c

__attribute__((weak)) int8_t get_permissive_hold_argos(uint16_t keycode, keyrecord_t *record) {
    return -1;  // Default: use Argos's setting
}

__attribute__((weak)) int8_t get_hold_on_other_key_press_argos(uint16_t keycode, keyrecord_t *record) {
    return -1;  // Default: use Argos's setting
}

__attribute__((weak)) int8_t get_retro_tapping_argos(uint16_t keycode, keyrecord_t *record) {
    return -1;  // Default: use Argos's setting
}

__attribute__((weak)) uint16_t get_quick_tap_term_argos(uint16_t keycode, keyrecord_t *record) {
    return 0;  // Default: use Argos's setting
}

// Argos owns these functions - user hook is checked FIRST
bool get_permissive_hold(uint16_t keycode, keyrecord_t *record) {
    int8_t user_val = get_permissive_hold_argos(keycode, record);
    if (user_val >= 0) return user_val;
    return settings.tapping_v2 & (1 << TAPPING_PERMISSIVE_HOLD_BIT);
}

bool get_hold_on_other_key_press(uint16_t keycode, keyrecord_t *record) {
    int8_t user_val = get_hold_on_other_key_press_argos(keycode, record);
    if (user_val >= 0) return user_val;
    return settings.tapping_v2 & (1 << TAPPING_HOLD_ON_OTHER_KEY_BIT);
}

bool get_retro_tapping(uint16_t keycode, keyrecord_t *record) {
    int8_t user_val = get_retro_tapping_argos(keycode, record);
    if (user_val >= 0) return user_val;
    return settings.tapping_v2 & (1 << TAPPING_RETRO_TAPPING_BIT);
}

uint16_t get_quick_tap_term(uint16_t keycode, keyrecord_t *record) {
    uint16_t user_val = get_quick_tap_term_argos(keycode, record);
    if (user_val > 0) return user_val;
