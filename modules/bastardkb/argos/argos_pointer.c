...
// TODO: this evaluates to true for the Dilemma trackball, but calls ARGOS_CHARYBDIS_DEFAULT_DPI_CONFIG_STEP
#ifdef POINTING_DEVICE_DRIVER_pmw3360
...
#elif defined(POINTING_DEVICE_DRIVER_digitizer) || defined(CIRQUE_PINNACLE_DIAMETER_MM) // dilemma v2/v3
    // new dpi is on 2 bytes:
    uint16_t new_dpi = command_data[0] | (command_data[1] << 8);
    // get the old DPI:
    uint16_t old_dpi = bk_pointing_device_get_pointer_default_dpi();
...
// TODO: this evaluates to true for the Dilemma trackball, but calls ARGOS_CHARYBDIS_DEFAULT_DPI_CONFIG_STEP
#elif POINTING_DEVICE_DRIVER_pmw3360
    // new dpi is on 2 bytes:
    uint16_t new_dpi = command_data[0] | (command_data[1] << 8);
    // get the old DPI:
    uint16_t old_dpi = bk_pointing_device_get_pointer_sniping_dpi();
